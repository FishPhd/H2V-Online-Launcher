    ...
    public partial class MainWindow
    {
        ...
        */
        ...
        private bool CheckVersion()
        {
            ...
            var versionLines = new WebClient().DownloadString(UpdateServer + "version.txt").Split(new[] { "\r\n", "\n" }, StringSplitOptions.None); //Grab version number from the URL constant
            ...
            _localVersion = File.Exists(Cfg.InstallPath + @"\xlive.dll") ? FileVersionInfo.GetVersionInfo(Cfg.InstallPath + @"\xlive.dll").FileVersion : "0.0.0.0"; //Grab xlive.dll file version number. File doesn't exist? 0.0.0.0
            _halo2Version = File.Exists(Cfg.InstallPath + @"\" + ProcessName + ".exe") ? FileVersionInfo.GetVersionInfo(Cfg.InstallPath + @"\" + ProcessName + ".exe").FileVersion : "0.0.0.0"; //Grab halo2.exe file version number. 
        ...
        private bool Load()
        {
            try
            {
                ...
                PlayerName.Text = Cfg.ConfigFile["profile name 1 ="] == " " ? "Player" : Cfg.ConfigFile["profile name 1 ="]; //Set textbox to "Player" for numbering TODO: Halo 2 names
                GameArguments.Text = Cfg.ConfigFile["arguments ="] == " " ? "Command Parameters" : Cfg.ConfigFile["arguments ="]; //Set textbox to load saved command parameters
        ...
        private void ButtonAction_Click(object sender, RoutedEventArgs e)
        {
            if ((string)ButtonAction.Content == "Play")
            {
                try
                {
                    if (Cfg.CheckIfProcessIsRunning(ProcessName) || Cfg.CheckIfProcessIsRunning(ProcessStartup)) //Kill halo2 or startup if its running (Should help with black screens)
                    {
            ...
            {
                if (Cfg.CheckIfProcessIsRunning(ProcessName) || Cfg.CheckIfProcessIsRunning(ProcessStartup)) //Might be redundant but we don't want crashes
                {
                    KillProcess(ProcessName);
                    KillProcess(ProcessStartup);
                }
            ...
            {
                string tmp = Environment.CurrentDirectory; //gets current directory of launcher
        ...
        {
            if (Cfg.CheckIfProcessIsRunning(ProcessName) || Cfg.CheckIfProcessIsRunning(ProcessStartup))// Check if halo 2 is running
